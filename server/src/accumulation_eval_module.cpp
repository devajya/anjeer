...
void AccumulationEvalModule::on_trade_event(const EvalTradeEvent& ev) {
    ...
    if (ev.buyer_slot  >= 0 && ev.buyer_slot  < cap) signed_deltas_[ev.buyer_slot][si]  += ev.qty;
    if (ev.seller_slot >= 0 && ev.seller_slot < cap) signed_deltas_[ev.seller_slot][si] -= ev.qty;
