...
// ─────────────────────────────────────────────────────────────────────────────
// T28–T30: PUT /players/me/feed
// ─────────────────────────────────────────────────────────────────────────────

TEST_CASE("T28 — PUT /players/me/feed with valid pref returns 200 + stored value",
          "[integration][http_lobby][feed]") {
    ensure_http_server_running();
    reset_db();
    const auto pl = make_player();

    for (const auto* pref : {"mbp1", "mbpn", "mbo"}) {
        const std::string body = std::string(R"({"feed_preference":")") + pref + "\"}";
        const auto res = http("PUT", "/players/me/feed", pl.cookie, body);
        REQUIRE(res.status == 200);
        const auto j = nlohmann::json::parse(res.body);
        CHECK(j["feed_preference"].get<std::string>() == std::string(pref));

        // Verify DB was updated.
        auto handle = dp().acquire();
        pqxx::work txn(handle.get());
        const auto stored = pr().get_feed_preference(txn, pl.id);
        txn.commit();
        CHECK(stored == std::string(pref));
    }
}

TEST_CASE("T29 — PUT /players/me/feed with invalid pref returns 400",
          "[integration][http_lobby][feed]") {
    ensure_http_server_running();
    reset_db();
    const auto pl = make_player();

    const auto res = http("PUT", "/players/me/feed", pl.cookie,
                          R"({"feed_preference":"bad_value"})");
    REQUIRE(res.status == 400);
    CHECK(res.error_code() == "invalid_feed_preference");
}

TEST_CASE("T30 — PUT /players/me/feed unauthenticated returns 401",
          "[integration][http_lobby][feed]") {
    ensure_http_server_running();
    reset_db();

    const auto res = http("PUT", "/players/me/feed", "", R"({"feed_preference":"mbp1"})");
    REQUIRE(res.status == 401);
}
