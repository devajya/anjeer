...
const F = {
  ...
  cardTop: 0.7, // notch top edge    (70vh)
};
...
const SUB_SECTIONS = [
  {
    ...
    headline: "Play in your browser",
    body: "No installs. Open a lobby, grab a seat, and start playing in seconds from any modern browser.",
    svg: <BrowserSVG />,
  },
  {
    number: "02",
    ...
    body: "More of a hacker? Use the CLI tool to join games programmatically. Pipe your strategy straight into the market, giving you more control for more complexity.",
    svg: <TerminalSVG />,
  },
  {
    number: "03",
    ...
    body: "Want to see how others play? Watch live order flow, delta tables, and eval signals from any seat without touching a hand.",
    svg: <SpectatorSVG />,
  },
  {
    number: "04",
    ...
    body: "Three difficulty tiers, three game modes — Easy, Medium, Hard. Bots fill empty seats so games always run, and if you're winning up the challenge.",
    svg: <BotsSVG />,
  },
];
...
export function ScrollTrackerSection({
  reducedMotion,
}: ScrollTrackerSectionProps) {
  ...
  useEffect(() => {
    if (reducedMotion) return; // Initial opacity: state 0 visible, rest hidden
            ...
            onUpdate: ({ progress }: { progress: number }) => {
              ...
              if (symbolRef.current) {
                symbolRef.current.textContent =
                  SUITS[Math.min(N - 1, Math.floor(progress * N))];
  ...
  if (reducedMotion) {
    return (
      <section
        className="scroll-tracker-section scroll-tracker-section--static"
  ...
  return (
    ...
    >
      <div ref={stickyRef} className="scroll-tracker-sticky">
        ...
        <div
          ref={indicatorRef}
          className="scroll-tracker-indicator"
          aria-hidden="true"
        >
          <span ref={symbolRef} className="scroll-tracker-indicator-symbol">
            ♠
          </span>
        </div>
