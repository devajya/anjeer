...
function asciiFbm(x: number, y: number, t: number): number {
  let v = 0,
    ...
    fy = 1;
  for (let i = 0; i < 3; i++) {
    v += amp * Math.sin(x * fx * 0.14 + y * fy * 0.11 + t * 0.9 + i * 1.7);
...
function asciiTopBoundaryY(px: number, rh: number): number {
  const base = 0.1;
  const shore =
    0.018 * Math.sin(px * 0.018 + 2.0) + 0.01 * Math.sin(px * 0.034 - 1.1);
...
function arcLerp(src: Vec2, dst: Vec2, t: number, h: number): Vec2 {
  return {
    ...
    y: lerp(src.y, dst.y, t) - Math.sin(Math.PI * t) * h,
  };
}
...
export function AutoAdvanceProgress({
  reducedMotion,
}: AutoAdvanceProgressProps) {
  const [activeIndex, setActiveIndex] = useState(0);
  const [progressPct, setProgressPct] = useState(0);
  const sectionRef = useRef<HTMLElement>(null);
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const chipStateRef = useRef<ChipRenderState>({
    positions: SEAT_POSITIONS.map((p) => ({ ...p })),
    anim: null,
  });
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e: MouseEvent) => {
      asciiMouseRef.current = { x: e.clientX, y: e.clientY };
    };
    ...
    const draw = (t: number) => {
      ...
      const mx = asciiMouseRef.current.x,
        my = asciiMouseRef.current.y;
      const cx = (mx - rect.left) / cellW,
        cy = (my - rect.top) / cellH;
      const margin = 24;
      const inside =
        mx >= rect.left - margin &&
        mx <= rect.right + margin &&
        my >= rect.top - margin &&
        my <= rect.bottom + margin;
      const chipCX = W * 0.72,
        chipCY = H * 0.5;
      ...
      for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
          ...
          const fadePanel = Math.max(
            0,
            Math.min(1, (px - boundary) / ASCII_FADE_PX),
          );
          const dxC = Math.max(0, Math.abs(px - chipCX) - chipHW);
          const dyC = Math.max(0, Math.abs(py - chipCY) - chipHH);
          const fadeChip = Math.min(
            1,
            Math.sqrt(dxC * dxC + dyC * dyC) / ASCII_FADE_PX,
          );
          const fadeTop = Math.max(
            0,
            Math.min(
              1,
              (py - asciiTopBoundaryY(px, H)) / ASCII_VERTICAL_FADE_PX,
            ),
          );
          const fadeBottom = Math.max(
            0,
            Math.min(
              1,
              (asciiBottomBoundaryY(px, H) - py) / ASCII_VERTICAL_FADE_PX,
            ),
          );
          ...
          const dx = x - cx,
            ...
            d = Math.sqrt(d2);
          const ripple = inside
            ? ASCII_RIPPLE_STR * Math.exp(-d2 / 80) -
              0.6 * Math.exp(-((d - ASCII_RIPPLE_RAD) ** 2) / 30)
            : 0;
          ...
          if (inside) {
            alpha =
              ASCII_BASE_OPACITY +
              (ASCII_SPOT_OPACITY - ASCII_BASE_OPACITY) *
                Math.exp(-d2 / spotR2);
          ...
          const huePos =
            (x * 0.1 + y * 0.07 + time * 0.12) % ASCII_PALETTE.length;
          ctx.globalAlpha = alpha;
          ctx.fillStyle =
            ASCII_PALETTE[Math.floor(Math.abs(huePos)) % ASCII_PALETTE.length];
    ...
    function drawFrame() {
      ...
      if (state.anim) {
        ...
        for (let i = 0; i < 4; i++) {
          ...
          if (elapsed < 0) {
            pos[i] = { ...srcPositions[i] };
            allDone = false;
          } else if (elapsed >= durationMs) {
            pos[i] = { ...dstPositions[i] };
          } else {
            pos[i] = arcLerp(
              srcPositions[i],
              dstPositions[i],
              easeInOut(elapsed / durationMs),
              40,
            );
            allDone = false;
          }
        ...
        if (allDone) {
          state.positions = dstPositions.map((p) => ({ ...p }));
          state.anim = null;
          onComplete?.();
        }
      }
  ...
  return (
    <section ref={sectionRef} className="auto-advance-progress">
      <div className="aap-inner">
        {!reducedMotion && (
          <canvas ref={asciiCanvasRef} className="aap-ascii-canvas" />
        )}
