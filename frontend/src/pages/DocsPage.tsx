...
type DocTab = 'start' | 'api' | 'bots'

const TABS: { id: DocTab; label: string }[] = [
  { id: 'start', label: 'Getting Started' },
  { id: 'api',   label: 'API Reference'   },
  { id: 'bots',  label: 'Bot Patterns'    },
]
...
anjeer close <CODE>           # leave and delete the lobby if empty`}</code></pre>
      ...
      <section id="ui-guide">
        ...
        <h3>Eval Panel</h3>
        <p>
          The eval panel (accessible via the Math tab during a game) shows live signals
...
ANJEER_LOBBY_CODE       # Lobby code your script is participating in`}</code></pre>
      ...
      {/* ── 6. Inbound Messages ─────────────────────────────────────── */}
      <section id="inbound">
        <h2>Inbound Messages (Server → Client)</h2>
...
// deltas[player_slot][suit_index]; suit order: 0=clubs 1=diamonds 2=hearts 3=spades`}</code></pre>
  ...
  suits: [{ suit, best_bid, best_ask, best_bid_slot, best_ask_slot }, ...] }`}</code></pre>
      ...
      {/* ── 7. Outbound Messages ────────────────────────────────────── */}
      <section id="outbound">
        <h2>Outbound Messages (Client → Server)</h2>
...
function BotPatternsTab() {
  return (
    <main className="docs__body">

      <section id="bot-patterns">
        <h2>Bot Design Patterns</h2>
        <p>
          The message schemas in API Reference describe individual events. This section describes how
...
export function DocsPage() {
  const [tab, setTab] = useState<DocTab>('start')
      ...
      <div className="docs__tabs" role="tablist">
        {TABS.map(t => (
          <button
            ...
            className={`docs__tab${tab === t.id ? ' docs__tab--active' : ''}`}
            onClick={() => setTab(t.id)}
          >
            {t.label}
          </button>
