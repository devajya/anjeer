...
my_orders:     dict = {}   # order_id → {suit, side, price}
in_round:      bool = False
rounds_played: int = 0
ws_ref = None
...
async def submit_order(suit: str, side: str, price: int):
    await ws_ref.send(json.dumps({"type": "submit_order", "suit": suit, "side": side, "price": price}))
...
async def on_player_hello(msg: dict):
    ...
    print(f"[hello] player_id={player_id}")

async def on_round_start(msg: dict):
    global hand, balance, my_slot, in_round, my_orders
    hand      = msg["hand"]
    balance   = msg["balance"]
    my_slot   = msg["player_slot"]
    in_round  = True
    my_orders = {}
    print(f"[round_start] slot={my_slot} hand={hand} balance={balance:.2f}")
...
async def on_trade(msg: dict):
    global my_orders
    my_orders = {}  # all books wipe on every trade
...
async def on_order_ack(msg: dict):
    my_orders[msg["order_id"]] = {"suit": msg["suit"], "side": msg["side"], "price": msg["price"]}
    print(f"[ack] {msg['side']} {msg['suit']}@{msg['price']} id={msg['order_id']}")
...
async def main():
    ...
    print(f"Connecting to {_ws} (lobby {LOBBY_CODE}) …")
